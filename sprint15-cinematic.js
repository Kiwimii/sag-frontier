(()=>{
  ...
  function init(){
    ...
    if(!story.faction){document.getElementById('sagIntro')?.classList.remove('sag-hidden');document.getElementById('sagFactionStage')?.classList.add('sag-hidden')}else{document.getElementById('sagIntro')?.classList.add('sag-hidden');document.getElementById('sagFactionStage')?.classList.add('sag-hidden');setTimeout(()=>openHQ('home'),450)}
  }
  window.SAGRecruitment={Core,get story(){return story},loadStory,saveStory,getBase,getProgression,saveContext,openHQ,closeHQ,setTab,registerTab,renderHome,showKiwimi,refreshMetrics,syncGameSave};
