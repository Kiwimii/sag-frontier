...
assert.ok(Core.LORE.some(item=>item.id==='iris-cataclysm'&&item.scope==='STAR ATLAS KANON'));
assert.ok(Core.LORE.some(item=>item.id==='sag-origin'&&item.scope==='SAG DAC'));
assert.ok(Core.LORE.some(item=>item.id==='black-signal'&&item.scope==='SAG FRONTIER FIKTION'));
...
assert.ok(story.loreUnlocked.includes('iris-cataclysm'));
assert.ok(story.loreUnlocked.includes('convergence-war'));
assert.ok(story.loreUnlocked.includes('starpath'));
assert.ok(story.loreUnlocked.includes('sag-matrix'));
assert.ok(story.loreUnlocked.length>=15);
...
assert.ok(Core.EVENTS.every(item=>item.choices.length===2));
